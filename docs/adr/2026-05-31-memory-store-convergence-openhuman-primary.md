...
- **P2 — migrate gbrain knowledge → adapter.** PGLite → SQLite data migration; repoint the `put_page`/`query`/`search` write+read paths (incl. the agent system-prompt instruction block) to the adapter; **retire the gbrain MCP server + the Bun runtime + PGLite dependency.**
  - *Progress (2026-06-01):* **P2b** page-data migration (done) · **P2a-1** Rust write-site dual-write + **P2a-2** LLM `put_page` write intercept (done) · **P2c-1** passive-recall read repoint + v2 re-sync (done) · **P2c-2** LLM read-tool repoint `get_page/list_pages/search/query` (done; `query` degraded to `recall_hybrid`, graph params dropped). All gated by `gbrain_dual_write_pages_enabled` / `gbrain_read_repoint_enabled` (default on, rollback by flag).
  - *P2c-3 (UI read repoint) folded into **P2d**:* the only UI gbrain reader is **`WikiView`**, a rich graph/version viewer that couples the 3 page-reads (`gbrain_list_pages/get_page/search`) with graph/version commands (`get_backlinks`/`traverse_graph`/`get_versions`/`revert_version`/`get_stats`/`find_orphans`/`full_graph`) that have **no adapter equivalent**. Repointing only the page-reads would split-brain the UI while gbrain must stay alive for the rest — so WikiView's fate (simplify to an adapter pages-browser dropping graph/version, **or** remove it) is decided at P2d alongside gbrain teardown, not as a standalone P2c slice. The substantive read repoint (the LLM's knowledge use) is complete via P2c-1/P2c-2.
