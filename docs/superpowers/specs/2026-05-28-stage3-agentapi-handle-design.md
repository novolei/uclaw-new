...
| | ⚠️ **Correction (2026-05-29, P3-3 recon)**: Both targets had shape gaps — ProviderService is a process-scope singleton (not a registry of providers), and HookBus has its own decision aggregation (Allow/Deny/AskUser) distinct from AgentApi's EventOutcome (Continue/Patch/Abort) with 13 different variants. The user-grilled Option E resolution: (a) AgentApi.emit() fan-outs to HookBus.dispatch_observe (when an Event ↔ HookEvent mapping exists), (b) new `emit_with_decision()` method maps HookDecision back to EventOutcome (Deny → Abort, Allow → Continue, AskUser → Abort with "askuser:" tagged reason), (c) P3-1's register_provider/provider() API replaced by set_provider_service/provider_service() (since there's only one ProviderService singleton). Full details + 13-variant translation table at [`2026-05-29-stage3-p3-provider-hookbus-bridge.md`](../plans/2026-05-29-stage3-p3-provider-hookbus-bridge.md). | | |
...
| | ⚠️ **Correction (2026-05-29, P3-4 recon)**: McpManager already implements 95% of the proposed "SubprocessPluginManager" surface (full JSON-RPC 2.0, MCP initialize/list_tools/call_tool/list_resources lifecycle, subprocess spawn + stdio transport, health tasks, audit DB persistence, McpToolProxy bridging MCP tools to the `Tool` trait — 1,925 LoC at `src/mcp.rs`). Plus `plugin_manifest/schema.rs` (P2-preserved) already has `PluginContribution { mcp_servers, skills, commands, tools, themes }`. User-grilled Option B (2026-05-29): build a thin **PluginDiscovery** layer + **manifest → AgentApi/McpManager registration** + **`uclaw` capability extension** in the MCP handshake, instead of duplicating 1,925 LoC. 6 tasks. Full plan: [`2026-05-29-stage3-p4-plugin-discovery-bridge.md`](../plans/2026-05-29-stage3-p4-plugin-discovery-bridge.md). | | |
