...
- 切换在 `AppShell` 层（见 §4.2）：`topLevelView === 'kaleidoscope' ? <KaleidoscopeShell/> : <workspace 三件套>`，一个 plain conditional，**瞬时切换无 cross-dissolve**
- **为什么没有 cross-dissolve：** 两个 surface 的 flex 结构不兼容 —— workspace 是 3 个 flex 兄弟节点，kaleidoscope 是 1 个 flex 子节点。在它们的共同父层套 `AnimatePresence` + `motion.div`：用 `display:contents` 则 opacity 动画不可见（contents 元素不绘制），用真实 wrapper div 则会改变 workspace 的 flex 布局造成回归。「功能正确 + workspace 零回归」优先于过场动画
- React 通过条件渲染天然卸载 / 重挂载对应 surface（避免状态串扰）
- **未来如果要过场动画**：让每个 shell 在自己 root 上做 mount-time fade-in（`KaleidoscopeShell` 内部模块切换已是此模式），entrance-only、不需要在切换点套 wrapper —— 留作 Phase 2 可选打磨
