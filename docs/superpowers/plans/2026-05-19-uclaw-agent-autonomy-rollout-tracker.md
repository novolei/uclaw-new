...
| global-control-plane-doc | pending | pending | harness architecture | `git diff --check`; `rg -n "HarnessSubject Coverage Matrix\\|Global Event Schema\\|Production Trace to Promotion Loop\\|Current Implementation Truth" docs/superpowers/specs/2026-05-19-uclaw-agent-autonomy-harness-design.md` | N/A | Pending | Reframes Harness as a global agentic runtime control plane covering Browser, Agent Loop, Agent Message, Context, Automation, Memory/gbrain, Skills, Prompts, Hooks, Permissions, Tools, Plugins, Coordinator, UI Projection, and Runtime Health. |
...
After the global control-plane docs PR, the next documentation-aligned implementation target is **Global Trace Envelope and Subject Registry**.

Reason: the harness substrate, browser identity, perception seam, boundary broker, memory/gbrain adapter, Browser parity suite, memory/gbrain eval suite, agent control-plane suite, UI reporting, and self-improvement gates are now implemented as early slices. The remaining architectural gap is that production events from agent messages, context assembly, automation, tools, permissions, memory/gbrain, browser tasks, and UI projection do not yet flow through one global episode model. Persisted history and live Browser smoke should follow this trace envelope work, not replace it.
