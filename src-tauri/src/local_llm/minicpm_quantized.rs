...
//! MiniCPM quantized Llama loader — VENDORED from huggingface/candle
//! `candle-transformers` 0.10.2 `src/models/quantized_llama.rs` (Apache-2.0 OR MIT),
//! with two patches so MiniCPM5-1B (head_dim=128 ≠ embedding_length/head_count=96) loads:
//!   1. `from_gguf`: head_dim is read from `llama.attention.key_length` (fallback to
//!      embedding_length/head_count for vanilla models).
//!   2. attention output reshape uses `n_head * head_dim` (not the residual `n_embd`),
//!      so the output projection (n_head*head_dim → embedding_length) gets the right width.
//! See docs/THIRD_PARTY.md + NOTICE. Upstream is unchanged except those two points
//! and the module-path rewrites required to live outside the candle crate.
//
// Vendored upstream — lints relaxed for upstream code we do not maintain.
#![allow(dead_code)]
#![allow(clippy::all)]
...
impl LayerWeights {
    ...
    fn forward_attn(
        ...
        // MiniCPM patch: concat of heads is n_head*head_dim (2048), not the residual n_embd (1536).
        // The o_proj (attention_wo) maps 2048→1536, so feed it the correct width.
...
impl ModelWeights {
    ...
    pub fn from_gguf<R: std::io::Seek + std::io::Read>(
        ...
        // MiniCPM patch: head_dim is explicit via key_length (128), NOT embedding_length/head_count (96).
        // For vanilla Llama models that don't set this key, fall back to the standard formula.
        let head_dim = match md_get("llama.attention.key_length") {
