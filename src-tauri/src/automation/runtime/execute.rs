...
/// Backward-compatible alias — existing callers of `AutomationDelegate` in
/// `service.rs` and the inline tests migrate to `HeadlessDelegate` directly;
/// this alias is kept only as a transitional shim and will be removed once
