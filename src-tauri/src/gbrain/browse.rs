...
pub fn parse_page_detail(json_text: &str) -> Result<PageDetail, GbrainError> {
    ...
    let mut detail: PageDetail = serde_json::from_value(v)
        .map_err(|e| GbrainError::ParseFailed(e.to_string()))?;
    detail.raw_markdown = crate::memory_graph::page_markdown::build_raw_markdown(&detail.frontmatter, &detail.compiled_truth);
    Ok(detail)
}
...
// `build_raw_markdown` and `split_frontmatter` were relocated to
// `crate::memory_graph::page_markdown` in Step 2d. Use that module directly.
...
mod tests {
    ...
    // build_raw_markdown and split_frontmatter tests relocated to
    // memory_graph::page_markdown::tests in Step 2d.
}
