...
//! Deterministic fallback summariser (openhuman port — Phase 3a).
//!
...
impl Summariser for InertSummariser {
    async fn summarise(
        ...
        tracing::debug!(
            ...
            inputs = inputs.len(),
            tokens = token_count,
            "[tree_source::summariser::inert] sealed (honest stub — LLM summariser derives entities/topics)"
        );
...
mod tests {
    ...
    async fn honest_stub_emits_no_entities_or_topics() {
        ...
        // not propagate children's labels — it emits empty vecs.
        let s = InertSummariser::default();
        let inputs = vec![
