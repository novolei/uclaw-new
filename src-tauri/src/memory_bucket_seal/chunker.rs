...
use crate::memory_bucket_seal::types::{approx_token_count, Chunk, Metadata, SourceKind};
...
pub fn chunk_markdown(input: &ChunkerInput, opts: &ChunkerOptions) -> Vec<Chunk> {
    ...
    let units: Vec<String> = match input.source_kind {
        ...
        SourceKind::Document => {
            ...
            tracing::debug!(
                source_id_hash = %redact(&input.source_id),
                len = input.markdown.len(),
                "memory_bucket_seal::chunker: document — paragraph split"
            );
    ...
    if matches!(input.source_kind, SourceKind::Document) {
        ...
        return units
            ...
            .map(|(idx, content)| {
                ...
                let id = crate::memory_bucket_seal::types::chunk_id(
                    input.source_kind,
                    &input.source_id,
                    seq,
                    &content,
    ...
    tracing::debug!(
        ...
        len = input.markdown.len(),
        units = units.len(),
        "memory_bucket_seal::chunker: chat/email — greedy pack"
    );
    ...
    if out.is_empty() {
        ...
        let id =
            crate::memory_bucket_seal::types::chunk_id(input.source_kind, &input.source_id, 0, "");
        out.push(Chunk {
            id,
            content: String::new(),
            metadata: input.metadata.clone(),
