...
use crate::browser::local_chromium_adapter::LocalChromiumProviderAdapter;
use crate::browser::playwright_cli::{playwright_cli_provider_status, PLAYWRIGHT_CLI_PROVIDER_ID};
use crate::browser::playwright_cli_adapter::{
    playwright_cli_action_for_browser_action, PlaywrightCliProviderAdapter,
    PlaywrightCliProviderAdapterConfig,
};
...
#[cfg(test)]
use crate::browser::playwright_cli::PlaywrightCliOfficialCommandConfig;
#[cfg(test)]
use crate::browser::playwright_cli_adapter::apply_preview_mirror_success;
...
impl BrowserProviderActionExecutor {
    ...
    pub async fn execute_routed_with_identity(
        ...
        let result = LocalChromiumProviderAdapter::new(&self.action_registry)
            .execute_action(session_id, identity_profile_id, action)
            .await?;
    ...
    async fn execute_playwright_cli_route(
        ...
        let adapter = PlaywrightCliProviderAdapter::new(
            &self.action_registry,
            PlaywrightCliProviderAdapterConfig {
                feature_flags: self.route_options.feature_flags,
                runtime_report: self.route_options.runtime_report.clone(),
                command: self.route_options.playwright_cli_command.clone(),
                cwd: self.route_options.playwright_cli_cwd.clone(),
            },
        );
        let action_result = match adapter
            .execute_action(request_id, session_id, identity_profile_id, action)
            .await
        {
            Ok(result) => result,
            Err(blocked) => {
