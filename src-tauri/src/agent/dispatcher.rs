...
use crate::agent::tools::tool::{
    execute_streaming_with_context, execute_tool_with_context, ToolExecutionContext, ToolOutput,
    ToolRegistry,
};
...
pub struct ChatDelegate {
    ...
    last_injected_fragments: std::sync::Mutex<Vec<crate::runtime::context::ContextArtifact>>,
...
fn render_context_fragments(fragments: &[crate::runtime::context::ContextArtifact]) -> String {
...
impl LoopDelegate for ChatDelegate {
    ...
    async fn execute_tool_calls(
        ...
        let mut parallel_batch: Vec<(ToolCall, serde_json::Value, ToolExecutionContext)> =
            Vec::new();
            ...
            match tool {
                Some(tool) => {
                    ...
                    let coalescer = if wants_stream {
                        let (sink, mut rx) =
                            crate::agent::tools::stream::ToolStreamSink::channel(256);
            ...
            while let Some(task_result) = join_set.join_next().await {
                ...
                match execute_result {
                    Ok(output) => {
                        ...
                        let raw_result_str =
                            serde_json::to_string(&output.result).unwrap_or_else(|_| "{}".into());
                    ...
                    Err(e) => {
                        ...
                        if let Some(ref store) = self.trajectory_store {
                            ...
                            if let Err(re) = store.record_turn(&record) {
                                tracing::warn!(
                                    "Failed to record parallel error trajectory turn: {re}"
                                );
                            }
                        }
