...
impl rand::RngCore for FixedRng {
    ...
    fn fill_bytes(&mut self, dst: &mut [u8]) {
        for b in dst.iter_mut() { *b = self.0 as u8; }
    }
...
fn compute_delay_base_sequence_no_jitter() {
    ...
    assert!((d1.as_millis() as i64 - 1_000).abs() < 100, "attempt 1: {:?}", d1);
    assert!((d2.as_millis() as i64 - 2_000).abs() < 100, "attempt 2: {:?}", d2);
    assert!((d3.as_millis() as i64 - 4_000).abs() < 200, "attempt 3: {:?}", d3);
    assert!((d4.as_millis() as i64 - 8_000).abs() < 400, "attempt 4: {:?}", d4);
    assert!((d5.as_millis() as i64 - 15_000).abs() < 800, "attempt 5: {:?}", d5);
    assert!((d6.as_millis() as i64 - 15_000).abs() < 800, "attempt 6 (capped): {:?}", d6);
}
