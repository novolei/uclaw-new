...
impl<R: tauri::Runtime> Tool for LoadSkillTool<R> {
    ...
    async fn execute(&self, params: serde_json::Value) -> Result<ToolOutput, ToolError> {
        ...
        let version = self
            ...
            .ok_or_else(|| {
                ToolError::Execution(format!("Skill '{}' has no active version", name))
            })?;
...
mod tests {
    ...
    async fn builtin_skill_loads_from_registry() {
        ...
        assert!(
            out.result["validation_hint"].is_null(),
            "builtin should return null for validation_hint"
        );
    }
    ...
    async fn returns_validation_hint_from_metadata() {
        ...
        let out = tool
            .execute(json!({ "name": "verify-skill", "reason": "test" }))
            .await
            .unwrap();
        assert_eq!(
            out.result["validation_hint"],
            "Re-run with --quiet and check exit code"
        );
        assert_eq!(out.result["provenance"], "learned");
    }
}
