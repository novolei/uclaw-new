...
export function SidePanel({ sessionId, sessionPath }: SidePanelProps): React.ReactElement {
  ...
  const attachSessionDir = React.useCallback(async (dirPath: string) => {
    const updated = await attachDirectory({ sessionId, directoryPath: dirPath })
  ...
  const handleDetachDirectory = React.useCallback(async (dirPath: string) => {
    try {
      const updated = await detachDirectory({ sessionId, directoryPath: dirPath })
      setAttachedDirsMap((prev) => {
        const map = new Map(prev)
  ...
  const attachWorkspaceDir = React.useCallback(async (dirPath: string) => {
    if (!workspaceSlug || !currentWorkspaceId) return
    const updated = await attachWorkspaceDirectory({ workspaceSlug, directoryPath: dirPath })
    setWsAttachedDirsMap((prev) => {
      const map = new Map(prev)
  ...
  const handleDetachWorkspaceDirectory = React.useCallback(async (dirPath: string) => {
    if (!workspaceSlug || !currentWorkspaceId) return
    try {
      const updated = await detachWorkspaceDirectory({ workspaceSlug, directoryPath: dirPath })
      setWsAttachedDirsMap((prev) => {
        const map = new Map(prev)
                        ...
                        <Tooltip>
                          <TooltipTrigger asChild>
                            <Button
                              ...
                              onClick={() => openFile(sessionPath).catch(console.error)}
                            >
                        ...
                        <Tooltip>
                          <TooltipTrigger asChild>
                            <Button
                              ...
                              size="icon"
                              className="h-5 w-5 flex-shrink-0"
                              onClick={() => openFile(workspaceFilesPath).catch(console.error)}
                            >
                              <ExternalLink className="size-2.5" />
                            </Button>
                          </TooltipTrigger>
...
function AttachedDirTree({ dirPath, onDetach, selectedPaths, onSelect, refreshVersion, onAddToChat }: AttachedDirTreeProps): React.ReactElement {
  ...
  React.useEffect(() => {
    if (expanded && loaded) {
      listAttachedDirectory(dirPath)
        .then((items: any[]) => setChildren(items))
        .catch((err: unknown) => console.error('[AttachedDirTree] 刷新失败:', err))
    }
...
function AttachedDirItem({ entry, depth, selectedPaths, onSelect, refreshVersion, onAddToChat }: AttachedDirItemProps): React.ReactElement {
  ...
  React.useEffect(() => {
    if (expanded && loaded && entry.isDirectory) {
      listAttachedDirectory(currentPath)
        .then((items: any[]) => setChildren(items))
        .catch((err: unknown) => console.error('[AttachedDirItem] 刷新子目录失败:', err))
    }
  }, [refreshVersion]) // eslint-disable-line react-hooks/exhaustive-deps
  ...
  const handleDoubleClick = (): void => {
    if (!entry.isDirectory) {
      openAttachedFile(currentPath).catch(console.error)
    }
  }
  ...
  const confirmRename = async (): Promise<void> => {
    ...
    try {
      await renameAttachedFile(currentPath, newName)
      // 更新本地显示
  ...
  const handleMove = async (): Promise<void> => {
    try {
      const result = await openFolderDialog()
      if (!result) return
      await moveAttachedFile(currentPath, result.path)
      // 移动后更新路径
          ...
          <DropdownMenu>
            <DropdownMenuTrigger asChild>
              ...
              <DropdownMenuContent align="start" className="w-40 z-[9999] min-w-0 p-0.5">
                ...
                <DropdownMenuItem
                  className="text-xs py-1 [&>svg]:size-3.5"
                  onSelect={() => showAttachedInFolder(currentPath).catch(console.error)}
                >
                  <FolderSearch />
                  在文件夹中显示
                </DropdownMenuItem>
                {!entry.isDirectory && (
                  <DropdownMenuItem
                    className="text-xs py-1 [&>svg]:size-3.5"
                    onSelect={() => openAttachedFile(currentPath).catch(console.error)}
                  >
                    <ExternalLink />
                    打开文件
                  </DropdownMenuItem>
