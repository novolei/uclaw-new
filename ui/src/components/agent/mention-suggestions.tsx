...
import { getWorkspaceCapabilities } from '@/lib/tauri-bridge'
  ...
  return createMentionSuggestion<SkillMentionItem>(
    {
      ...
      fetchItems: async (slug, q) => {
        const caps = await getWorkspaceCapabilities(slug)
        return caps.skills
          .filter((s: any) => s.enabled)
          .filter((s: any) => !q || s.name.toLowerCase().includes(q) || (s.slug ?? '').toLowerCase().includes(q))
          .map((s: any) => ({ id: s.slug, name: s.name, description: s.description }))
      },
  ...
  return createMentionSuggestion<McpMentionItem>(
    {
      ...
      fetchItems: async (slug, q) => {
        ...
        return caps.mcpServers
          ...
          .map((s: any) => ({ id: s.name, name: s.name, type: s.type }))
      },
      keyExtractor: (item) => item.id,
      renderItem: (item) => (
        <>
